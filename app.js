...
function computeBetweennessCentrality(nodeIds, adjacency) {
  ...
  nodeIds.forEach((source) => {
    ...
    while (stack.length > 0) {
      ...
      if (vertex !== source) {
        betweenness.set(vertex, betweenness.get(vertex) + dependency.get(vertex));
      }
    }
  });
  ...
  if (!isDirectedMode()) {
    nodeIds.forEach((id) => {
      betweenness.set(id, betweenness.get(id) / 2);
    });
  }
...
function renderMetrics() {
  ...
  const betweenness = computeBetweennessCentrality(nodeIds, adjacency);
  ...
  nodeIds.forEach((startId) => {
    ...
    if (closenessDistanceSum > 0) {
      ...
      const closeness = (closenessReachable / nMinusOne) * (closenessReachable / closenessDistanceSum);
      closenessMap.set(startId, closeness);
    } else {
      closenessMap.set(startId, 0);
    }
  });
  ...
  nodeIds.forEach((nodeId) => {
    ...
    row.innerHTML = `
      ...
      <td>${closenessMap.get(nodeId).toFixed(3)}</td>
      <td>${betweenness.get(nodeId).toFixed(3)}</td>
    `;
